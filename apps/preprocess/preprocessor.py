...
class Preprocessor:
    ...
    def feature_encoding(self, data):
        ...
        try:
            ...
            for col in self.new_data.columns:
                self.new_data = pd.get_dummies(
                    self.new_data, columns=[col], prefix=[col], drop_first=True
                )
    ...
    def split_features_label(self, data, label_name):
        ...
        try:
            ...
            self.logger.exception(
                "Exception raised while splitting features and label:" + str(e)
            )
            raise Exception()
